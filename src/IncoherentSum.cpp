...
real_t IncoherentSum::prob_unnormalisedNoCache( const Event& evt ) const
{
  return utils::get<0>( real_t(std::accumulate( m_matrixElements.begin(), 
          ...
          real_t(0), 
          [&evt]( const auto& a, const auto& b ){ return a + std::norm(b.coefficient * b(evt));} )) );
}
...
/*
void IncoherentSum::normaliseAmps(const std::vector<std::string> exclude, const std::vector<std::string> combine){
    //prepare();
    for ( unsigned int i = 0; i < m_matrixElements.size(); ++i ) {
